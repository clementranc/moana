...
class SampledPosterior:
            ...
            for j in range(N):
                if j < i:
                    ...
                    a, b, c, d = moana.corner.compute_2dcontours(
                        x,
                        y,
                        bins=[bins[labels[j]+labels[i]][0], bins[labels[j]+labels[i]][1]],
    ...
    def scatter_plot_scaling(self, width: float = 2 * 84 / 25.4, 
                ...
                if j > i:
                    ...
                    if j == 0:
                        if i==0:
                            ...
                            ax[i][j].set_ylabel(rf"${label_names[labels[i]]}$", labelpad=0)
                    else:
                        ax[i][j].set_ylabel("")
                        if not i==j: ax[i][j].set_yticklabels([])
                    if (j <= i) and display_plot_coords:
                        ax[i][j].annotate(f"({i}, {j})", (0.05, 0.05), xycoords='axes fraction', c='k', 
                                size=11, weight=500, ha="left", va="bottom",
                                bbox=dict(boxstyle='square, pad=0', fc='None', ec='None'))
                    if not i==j:
                        if credible_intervals:
                            levels = np.concatenate([hist_levels[i][j], [hist[i][j].max()*(1+1e-6)]])
                            if fill_ci:
                                ax[i][j].contourf(xhist[i][j], yhist[i][j], hist[i][j].T, levels, **contourf_opts)
                            else:
                                ax[i][j].contour(xhist[i][j], yhist[i][j], hist[i][j].T, levels, **contourf_opts)
                        if show_samples:
                            for id_dchi2 in range(len(dchi2_list)-1):
                                cond = samples.reject == 1
                                samples.loc[cond,'reject'] = 0
                                samples.loc[((samples.dchi2 < dchi2_list[id_dchi2+1]) | (samples.dchi2 >= dchi2_list[id_dchi2])), 'reject'] = 1
                                cond = samples.reject == 0
                                ax[i][j].scatter(samples[cond][labels[j]].values, samples[cond][labels[i]].values, s=s_list[id_dchi2], 
                                        facecolors=colors[id_dchi2], marker='o', alpha=alpha[id_dchi2], linewidths=0, zorder=-100+id_dchi2)
                        # Limits
                        ...
                        if not ax_options[labels[i]][1] == None:
                            ax[i][j].set_ylim(ax_options[labels[i]][1][0], ax_options[labels[i]][1][1])
                        ...
                        if diagonal=='chi2':
                            ...
                            for k in range(100-1):
                                mask = (samples[labels[j]] > x[k]) & (samples[labels[j]] <= x[k+1])
                                if mask.sum() > 0:
                            ...
                            if display_1sigma:
                                ...
                                ax[i][i].errorbar(x, y, xerr=xerr, marker='o', ms=1, lw=0.5, capsize=0, color='k')
